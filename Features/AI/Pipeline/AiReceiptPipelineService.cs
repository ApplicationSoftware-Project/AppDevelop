...
public sealed class AiReceiptPipelineService
{
        ...
        CancellationToken ct = default)
    {
        ...
        var step1 = await kernel.InvokeAsync(
            ...
            ct);
        var normalizedText = step1.ToString();
        ...
        var step2 = await kernel.InvokeAsync(
            ...
            ct);
        var parsedJson = step2.ToString();

        var parsedInfo = TryDeserialize<ParsedReceiptInfo>(parsedJson)
        ...
        var category = AiCategoryCatalog.Options
            .Contains(classification.Category, StringComparer.OrdinalIgnoreCase)
            ? classification.Category
            : "기타";
        var confidence = Math.Clamp(classification.Confidence, 0d, 1d);
