...
// 사용자 확정 피드백: AI 추천 결과를 사용자가 확정하여 정확도 측정 데이터로 저장
app.MapPost("/api/ai/confirm-category", async Task<Results<Ok<ConfirmCategoryResult>, ValidationProblem, NotFound, ProblemHttpResult>> (ConfirmCategoryRequest request, App.Data.AppDbContext db, ILogger<Program> logger) =>
{
...
// AI 정확도 현황 조회: 누적 피드백 기준 정확도 확인
app.MapGet("/api/ai/accuracy", async Task<Ok<AiAccuracyResult>> (App.Data.AppDbContext db) =>
{
...
// AI 일별 정확도 조회: 최근 N일 기준 정확도 추이 확인
app.MapGet("/api/ai/accuracy/daily", async Task<Results<Ok<AiAccuracyDailyResult>, ValidationProblem>> (int days, App.Data.AppDbContext db) =>
{
    if (days < 1 || days > 365)
    {
        return TypedResults.ValidationProblem(new Dictionary<string, string[]>
...
// AI 주별 정확도 조회: 최근 N주 기준 정확도 추이 확인
app.MapGet("/api/ai/accuracy/weekly", async Task<Results<Ok<AiAccuracyWeeklyResult>, ValidationProblem>> (int weeks, App.Data.AppDbContext db) =>
{
...
// AI 월별 정확도 조회: 최근 N개월 기준 정확도 추이 확인
app.MapGet("/api/ai/accuracy/monthly", async Task<Results<Ok<AiAccuracyMonthlyResult>, ValidationProblem>> (int months, App.Data.AppDbContext db) =>
{
